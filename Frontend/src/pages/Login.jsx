...
const Login = () => {
  ...
  // If user signs in via redirect flow, navigate after auth state updates
  useEffect(() => {
    ...
    if (user) {
      ...
      const goTo = role === 'admin' || fallbackAdmin ? '/admin' : '/profile';
      console.log('[Login] Auth state ready. Redirecting to:', goTo, 'role:', role, 'email:', email);
      navigate(goTo, { replace: true });
    }
  }, [user, role, authLoading, navigate]);

  // Expose debug trigger to manually start Google login from console
  useEffect(() => {
    // eslint-disable-next-line no-console
    console.log('[Login] Component mounted. Debug: window.__loginWithGoogle available');
    window.__loginWithGoogle = handleGoogleLogin;
    return () => { try { delete window.__loginWithGoogle; } catch (_) {} };
  }, []);
  ...
  const handleEmailLogin = async (e) => {
    ...
    try {
      ...
      console.error('[Login] Email login error:', { code: err?.code, message: err?.message });
      const code = err?.code || '';
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (!user) {
        ...
        return; // User will be redirected, and useEffect above will handle navigation after return
      }
      ...
      console.error('[Login] Google sign-in error:', { code: err?.code, message: err?.message, name: err?.name });
