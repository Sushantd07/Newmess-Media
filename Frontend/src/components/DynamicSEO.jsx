...
const DynamicSEO = ({ type, identifier, tab }) => {
  ...
  const getPageInfo = () => {
    ...
    // Home page
    if (currentPath === '/') {
      return { type: 'home', identifier: 'home', tab: undefined };
    }
    ...
    if (currentPath === '/category') {
      ...
      return { type: 'all-categories', identifier: 'all-categories', tab: undefined };
    }
  ...
  const pageInfo = useMemo(() => getPageInfo(), [location?.pathname, JSON.stringify({ type, identifier, tab })]);
  ...
  }, [type, identifier, tab, seo, isActive]);
  ...
  }, [actualType, actualIdentifier, actualTab, isActive, location?.pathname]);
  ...
  useEffect(() => {
    const handleUpdated = (e) => {
      ...
      if (payload.type === type && payload.identifier === identifier) {
        if (!tab || !payload.tab || payload.tab === tab) {
          dbg('[DynamicSEO] Updating SEO data:', payload);
          setSeo(payload);
        }
      }
    };
