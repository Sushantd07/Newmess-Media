...
const DynamicSEOSafe = ({ type, identifier, tab, ssrData }) => {
  ...
  useEffect(() => {
    if (ssrData) {
      console.log('[DynamicSEOSafe] Using SSR data:', ssrData);
      setSeo(ssrData);
      setLoading(false);
      return;
    }
    ...
    const fetchSEO = async () => {
      try {
        setLoading(true);
        console.log('[DynamicSEOSafe] Fetching SEO for:', { actualType, actualIdentifier, actualTab });
        ...
        console.log('[DynamicSEOSafe] SEO data received:', data);
  ...
  useEffect(() => {
    if (seo && actualType && actualIdentifier) {
      const generateSchema = async () => {
        try {
          console.log('[DynamicSEOSafe] Generating dynamic schema for:', { actualType, actualIdentifier, actualTab });
          ...
          console.log('[DynamicSEOSafe] Dynamic schema generated and injected');
        } catch (error) {
  ...
  // Safe defaults - handle case when seo is null
  const defaults = {
    ...
    canonical: automaticSEO.canonical,
    robots: automaticSEO.robots,
    ...
    ogUrl: automaticSEO.ogUrl || automaticSEO.canonical,
    twitterCard: automaticSEO.twitterCard || 'summary_large_image',
  };

  // Debug logging
  console.log('[DynamicSEOSafe] Rendering Helmet with:', {
    seoData: seo,
    defaults,
    loading
  });
